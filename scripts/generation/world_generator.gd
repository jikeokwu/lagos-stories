...
func _assign_religious_orgs_to_npcs() -> int:
	...
	for org in all_orgs_debug:
		if org.get("type") == "religious" or org.get("category") == "religious":
			religious_orgs_debug.append({"id": org.id, "name": org.name, "type": org.get("type"), "category": org.get("category"), "district": org.get("district_id", "unknown")})
...
func _find_or_create_religious_org(district: String, religion: String, org_cache: Dictionary) -> String:
	...
	for org in all_orgs:
		...
		var is_religious = org_type == "religious" or org_type in ["christian_churches", "mosques", "traditional_shrines"]
		
		if not is_religious:
			# Check computed_values for category
			var cv = org.get("computed_values")
			if cv is String and cv != "" and cv != "{}":
				var parsed = JSON.parse_string(cv)
				if parsed is Dictionary:
					is_religious = parsed.get("category") == "religious"
		
		if is_religious:
			# Check computed_values for district and subcategory match
			var cv = org.get("computed_values")
			var org_district = ""
			var org_subcategory = ""
			...
			if cv is String and cv != "" and cv != "{}":
				var parsed = JSON.parse_string(cv)
				if parsed is Dictionary:
					org_district = parsed.get("district_id", "")
					org_subcategory = parsed.get("subcategory", "")
			
			# Match by district and subcategory (or name fallback)
			var district_match = (org_district == district or org_district == "")
			var type_match = (org_subcategory == expected_subcategory) or (religion.to_lower() in org.name.to_lower())
