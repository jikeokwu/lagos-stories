...
func _generate_world():
	# Update target to 1000 for better statistical analysis
	var target_npcs = 1000
	...
	_generate_family_frames(target_npcs)
	...
	_pass5_generate_singles(target_npcs)
	...
	print("7. Filling organization positions (Phase 5 - Multi-Pass)...")
	step_start = Time.get_ticks_msec()
	_fill_organization_positions_multipass()
	timings["org_filling"] = (Time.get_ticks_msec() - step_start) / 1000.0
	print("✅ Organization positions filled (%.3fs, O(n*m))\n" % timings["org_filling"])
...
func _create_world_state():
	DB.initialize_world_state(str(rng.randi()), "2025-01-01", "08:00")

func _generate_districts_dynamic():
	# 1. Determine number of districts (User config hook)
	var target_districts = rng.randi_range(5, 7)
	print("   🌍 Generating %d dynamic districts..." % target_districts)
...
func _generate_family_frames(target_npcs: int):
	var start_time = Time.get_ticks_msec()
	var districts = ["university_area", "market_district", "central", "waterfront", "industrial"]
	var template_names = []
	...
	# Generate family frames until we can accommodate target NPCs
	var potential_npc_count = 0
	var family_id = 0
	
	print("   📦 Generating family frames for ~%d NPCs..." % target_npcs)
	
	while potential_npc_count < target_npcs:
		family_id += 1
		...
		if family_frames.size() % 5 == 0:
			var progress = float(potential_npc_count) / float(target_npcs) * 100.0
...
func _pass5_generate_singles(target_npcs: int):
	var start_time = Time.get_ticks_msec()
	var created_count = 0
	var districts = ["university_area", "market_district", "central", "waterfront", "industrial"]
	
	# Calculate how many more NPCs we need
	var remaining = target_npcs - stats.npcs
	
	if remaining <= 0:
		print("   ✅ Pass 5: Target reached, no singles needed")
		return
...
func _create_relationship(npc_id: String, target_id: String, rel_type: String, strength: int):
	var affection = strength + rng.randi_range(-15, 15)
	var trust = strength + rng.randi_range(-10, 10)
	var respect = strength + rng.randi_range(-10, 10)
	
	DB.create_relationship(npc_id, target_id, rel_type, affection, trust, 0, respect)

func _create_locations_need_based():
	var start_time = Time.get_ticks_msec()
	
	# 1. Calculate Demand
	# Housing: 1 unit per family
	var housing_units_needed = stats.families
	# Commercial: 1 unit per organization
	var commercial_units_needed = organization_frames.size()
	
	# Add buffers (20% vacancy)
	var target_housing_units = int(housing_units_needed * 1.2)
	var target_commercial_units = int(commercial_units_needed * 1.2)
	
	print("   🏠 Housing Demand: %d units (Families: %d)" % [target_housing_units, stats.families])
...
## Phase 2B: Generate Organization Frames (structures with positions, no NPCs yet)
func _generate_organization_frames():
	...
	var districts = ["university_area", "market_district", "central", "waterfront", "industrial"]
	var org_id = 0
	var num_orgs = rng.randi_range(15, 25)
	
	# Category weights (pull from template or use defaults)
	var category_types = ["religious", "business", "education", "government", "social", "healthcare"]
	var category_weights = [0.25, 0.25, 0.15, 0.15, 0.10, 0.10]
	
	for i in range(num_orgs):
		org_id += 1
		
		# Select category
		var category_idx = Utils.weighted_random(category_types, category_weights)
		var category = category_types[category_idx]
		...
		# Select subcategory (e.g., christian_churches, mosques, etc.)
		var subcategory_keys = category_data.keys()
		if subcategory_keys.is_empty():
			continue
		var subcategory_key = subcategory_keys[rng.randi() % subcategory_keys.size()]
		...
		# Verify template_data is a dictionary
		if not (template_data is Dictionary):
			push_error("❌ template_data is not a Dictionary! Type: %s" % typeof(template_data))
			continue
		
		# Select size based on template distribution
		var org_size = "medium"
		var member_range = [40, 150]
		if template_data.has("size_distribution"):
			...
			for size_key in size_dist.keys():
				...
				# Handle both Dictionary format and direct access
			...
			if sizes.size() > 0:
				...
				# Get member range from selected size
				...
				elif size_entry is Array:
					# If it's an array, it might be [weight, [min, max]]
					if size_entry.size() >= 2 and size_entry[1] is Array:
		...
		var org_name = _generate_org_name_from_template(template_data, category, org_id)
		...
		var org_frame = {
			...
			"district": district,
			"founded": 2025 - rng.randi_range(5, 30),
			"location_id": null,
			"legal_status": "registered" if category != "social" else "informal",
			"pillars": ["service", "community"],
			"reputation": {
			...
			"computed_values": {},
			"positions": positions,  # Position structure from template
			"filled_positions": []  # Will be filled in Phase 5
		}
		
		organization_frames.append(org_frame)
		
		# Create basic DB record (without filled positions yet)
		var org_data = {
			"id": org_frame.id,
			"name": org_frame.name,
			"type": org_frame.type,
			...
			"computed_values": JSON.stringify(org_frame.computed_values)
		}
...
func _fill_organization_positions_multipass():
	if organization_frames.is_empty():
		print("   ⚠️ No organization frames to fill")
		return
	
	# Get all adult NPCs
	var all_npcs = DB.get_all_npcs(true)
	if all_npcs.is_empty():
		print("   ⚠️ No adult NPCs to assign to organizations")
		return
	
	# Tracking: { npc_id: { "employment": 0, "religious": 0, "social": 0, "criminal": 0 } }
	var npc_assignments = {}
	
	# Initialize tracking for all NPCs AND build district index
	var npcs_by_district = {}
	for npc in all_npcs:
		npc_assignments[npc.id] = {
			"employment": 0,
			"religious": 0,
			"social": 0,
			"criminal": 0
		...
		# Index by district
		var district = npc.identity.get("district", "unknown")
	...
	for org in organization_frames:
		var meta = _get_org_meta_category(org.category)
		if orgs_by_meta.has(meta):
			orgs_by_meta[meta].append(org)
	...
	print("   📋 Filling positions for %d organizations..." % organization_frames.size())
	
	# Process phases
	var phases = ["employment", "religious", "social", "criminal"]
	for phase in phases:
		var phase_orgs = orgs_by_meta[phase]
		if phase_orgs.is_empty():
			continue
			
		print("   Phase: %s (%d orgs)..." % [phase.capitalize(), phase_orgs.size()])
		_fill_org_group_positions(phase_orgs, npcs_by_district, phase, category_limits[phase], npc_assignments, fill_stats)

	# Print final stats
	var fill_rate = (float(fill_stats.filled) / fill_stats.total * 100.0) if fill_stats.total > 0 else 0.0
	print("   ✅ Filled %d/%d positions (%.1f%% fill rate)" % [fill_stats.filled, fill_stats.total, fill_rate])
	print("      Leadership: %d | Mid-level: %d | Entry: %d" % [fill_stats.leadership, fill_stats.mid, fill_stats.entry])
	
	# Show participation stats
	var participating_npcs = 0
	for npc_id in npc_assignments:
		var total_roles = 0
		for cat in npc_assignments[npc_id]:
			total_roles += npc_assignments[npc_id][cat]
		if total_roles > 0:
			participating_npcs += 1
			
	var part_rate = (float(participating_npcs) / all_npcs.size() * 100.0) if all_npcs.size() > 0 else 0.0
	print("      NPCs with ANY org positions: %d/%d (%.1f%% participation)" % [participating_npcs, all_npcs.size(), part_rate])
...
func _find_or_create_school(district: String, edu_level: String, school_cache: Dictionary) -> String:
	...
	for org in all_orgs:
		if org.type == "education" and org.get("district", "") == district:
	...
	var district_names = {
		"university_area": "University District",
		"market_district": "Market District",
		"central": "Central",
		"waterfront": "Waterfront",
		"industrial": "Industrial"
	}
	var district_name = district_names.get(district, district.capitalize())
...
func _assign_religious_orgs_to_npcs() -> int:
	...
	for npc in all_npcs:
		...
		if religious_path == "" or religious_path == "none" or religious_path == "None":
			continue
		...
		for membership in memberships:
			...
			if org != null and org.type == "religious":
...
func _find_or_create_religious_org(district: String, religion: String, org_cache: Dictionary) -> String:
	...
	# Search for existing religious org of this type in district
	var all_orgs = DB.get_all_organizations()
	for org in all_orgs:
		if org.type == "religious":
			# Simple heuristic: check if religion type matches org name
			if religion.to_lower() in org.name.to_lower():
...
func _assign_families_to_housing():
	...
	if residential_units.is_empty():
		push_error("❌ No residential units found!")
		return
...
func _assign_organizations_to_locations():
	...
	if commercial_units.is_empty():
		push_error("❌ No commercial units found!")
		return
	...
	for i in range(landlord_count):
		...
		var npc_data = {
			...
			"identity": JSON.stringify({
				...
				"religious_path": {"religion": _random_religion(tribe), "active": true},
...
func _assign_commercial_ownership() -> Dictionary:
	...
	for unit in units:
		...
		if unit.org_id == null:
			...
			if unit.org_assets:
				var assets = JSON.parse_string(unit.org_assets)
...
func _generate_relationship_events(start_year: int):
	...
	for rel in relationships:
		...
		var event_id = "evt-met-%s-%s" % [rel.source_npc_id, rel.target_npc_id]
...
func _validate_relationship_symmetry():
	# Ensure relationships have bidirectional entries (except specific types)
	var one_way_types = ["parent", "child", "landlord", "tenant", "boss", "subordinate"]
	var missing_rels = DB.get_asymmetric_relationships(one_way_types, 10000)
	...
	if missing_rels.size() > 0:
		validation_stats.issues_found += missing_rels.size()
		# Create missing reverse relationships with OPPOSITE type
		...
		print("      🔧 Created %d missing reverse relationships" % missing_rels.size())
